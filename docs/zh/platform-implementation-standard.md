...
- 若 `showInlineSuggestion()` 替换了当前已显示的建议，旧建议实例 MUST 先完成 `dismissed`，新建议才可成为当前建议
...
| 资源释放路径 | **MUST** | 平台 MUST 提供一种能最终释放 `EditorCore` 及其 native / C++ 侧资源的机制；释放时机 MAY 绑定显式 `dispose()` / `close()`、宿主管理生命周期、平台等价清理钩子或其他平台惯用机制 |
...
> 步骤 1-3 的内部顺序 MAY 调整，但 MUST 在步骤 4 之前完成。步骤 4 MUST 在步骤 5 之前完成。标准不再强制要求步骤 4 必须与控件永久移除这一特定时机绑定。
