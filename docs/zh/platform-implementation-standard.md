...
| **Adornment** | `StyleSpan`, `SpanLayer`, `InlayHint`, `InlayType`, `PhantomText`, `CodeLensItem`, `FoldRegion`, `GutterIcon`, `Diagnostic`, `IndentGuide`, `BracketGuide`, `FlowGuide`, `SeparatorGuide`, `SeparatorStyle`, `TextStyle` | 装饰数据类型 |
...
| **Event** | 类型安全事件机制、`EditorEvent`、`TextChangedEvent`、`CursorChangedEvent`、`SelectionChangedEvent`、`ScrollChangedEvent`、`ScaleChangedEvent`、`DocumentLoadedEvent`、`FoldToggleEvent`、`GutterIconClickEvent`、`InlayHintClickEvent`、`CodeLensClickEvent`、`LongPressEvent`*(仅移动端)*、`DoubleTapEvent`、`ContextMenuEvent`*(桌面端及跨平台 UI 框架)*；若采用显式事件总线 / 监听器模式，推荐 `EditorEventBus`、`EditorEventListener` | 事件系统 |
...
`DecorationResult` 包含 12 种装饰数据，每种数据都有对应的 `ApplyMode`（默认 `MERGE`）。数据类型 MUST 使用 Core 层定义的标准类型（如 `StyleSpan`、`InlayHint`、`CodeLensItem`、`Diagnostic` 等）。
...
平台在暴露装饰能力集合时，MUST 在 `DecorationType` 中包含 `CODELENS`。
...
| `active` | boolean | **MUST** | 可点击 run 的渲染期交互激活态，例如 `CODELENS` |
