...
| **Decoration** | `DecorationProvider`, `DecorationProviderManager`, `DecorationReceiver`, `DecorationContext`, `DecorationResult`, `DecorationType` | 装饰提供者系统 |
| **Completion** | `CompletionProvider`, `CompletionProviderManager`, `CompletionContext`, `CompletionItem`, `CompletionResult`, `CompletionReceiver` | 补全提供者系统 |
| **Event** | `EditorEventBus`, `EditorEventListener`, `EditorEvent`, `TextChangedEvent`, `CursorChangedEvent`, `SelectionChangedEvent`, `ScrollChangedEvent`, `ScaleChangedEvent`, `DocumentLoadedEvent`, `FoldToggleEvent`, `GutterIconClickEvent`, `InlayHintClickEvent`, `LongPressEvent`*(仅移动端)*, `DoubleTapEvent`, `ContextMenuEvent`*(桌面端及跨平台 UI 框架)* | 事件系统 |
| **NewLine** | `NewLineActionProvider`, `NewLineActionProviderManager`, `NewLineAction`, `NewLineContext` | 换行动作提供者系统 |
| **Copilot** *(SHOULD)* | `InlineSuggestion`, `InlineSuggestionListener`; MAY: `InlineSuggestionController` | 内联建议数据 + 回调；Controller 为可选实现方式 |
| **Selection** *(MAY, 仅移动端)* | `SelectionMenuController` 及相关类型 | 选区菜单（桌面端 MAY 省略） |
...
| `DecorationReceiver` | C#/TS/Kotlin: `IDecorationReceiver`; OC: `SEDecorationReceiver` | 回调接口 |
| `CompletionProvider` | C#/TS/Kotlin: `ICompletionProvider`; OC: `SECompletionProvider` | 提供者接口 |
| `CompletionReceiver` | C#/TS/Kotlin: `ICompletionReceiver`; OC: `SECompletionReceiver` | 回调接口 |
| `NewLineActionProvider` | C#/TS/Kotlin: `INewLineActionProvider`; OC: `SENewLineActionProvider` | 提供者接口 |
| `EditorEventListener` | C#/TS/Kotlin: `IEditorEventListener`; OC: `SEEditorEventListener` | 监听器接口 |
...
- 第 3.2 节的 API MUST 直接在控件入口类（如 `SweetEditor`、`SweetEditorView`、`SweetEditorControl`）上暴露为公共方法
...
- `SweetEditorController` MUST 暴露第 3.2 节定义的全部 API
...
| API 一致性 | **MUST** | Controller 上暴露的方法签名 MUST 与第 3.2 节的 Public API 表一致（方法名、参数、返回值） |
...
| 设置补全项视图工厂 | `setCompletionItemViewFactory(factory)` | — |
| **样式** | | |
| 注册文本样式 | `registerTextStyle(id, ...)` | — |
| 清除所有装饰 | `clearAllDecorations()` | — |
| **事件** | | |
| 订阅事件 | `subscribe(eventType, listener)` | — |
| 取消订阅 | `unsubscribe(eventType, listener)` | — |
| **刷新** | | |
| 刷新 | `flush()` | — |
| **查询** | | |
...
> **通用规则：**
> - `provideDecorations` 和 `provideCompletions` MUST 使用 **Receiver 回调模式**，以同时支持同步和异步返回：
>   - **同步**：在 `provide*()` 方法体内直接调用 `receiver.accept(result)` 后返回
>   - **异步**：在 `provide*()` 方法体内启动后台线程/协程，稍后调用 `receiver.accept(result)`，可多次调用
>   - `receiver.isCancelled()` 用于长时间计算中的提前取消检查
...
| `textChanges` | List\<TextChange\> | 本次刷新周期累积的文本变更；空列表表示非文本变更触发 |
...
> `NewLineActionProvider` 不使用 Receiver 模式，因为换行动作需要同步返回。
...
所有平台 MUST 提供 `EditorEventBus`，包含：
- `subscribe(eventType, listener)` — 为特定事件类型注册监听器
- `unsubscribe(eventType, listener)` — 移除监听器
- `publish(event)` — 向所有已注册监听器分发事件
- `clear()` — 移除所有监听器
...
SweetEditor 的所有公共 API 默认在 UI 线程（主线程）上调用和回调。

| 规则 | 约束级别 | 说明 |
|---|---|---|
| API 调用线程 | **MUST** | `EditorCore` 和 `SweetEditor` / `SweetEditorController` 的所有公共方法 MUST 在 UI 线程调用。从非 UI 线程调用属于未定义行为 |
| 事件回调线程 | **MUST** | `EditorEventBus.publish()` 分发的所有事件回调 MUST 在 UI 线程执行 |
| Provider 调用线程 | **MUST** | `provideDecorations()` 和 `provideCompletions()` MUST 在 UI 线程被调用 |
| Provider 异步回调线程 | **MUST** | `receiver.accept()` 可以在任意线程调用，但 Manager 在将结果应用到 Core 时 MUST 切换回 UI 线程 |
| `buildRenderModel()` | **MUST** | MUST 在 UI 线程调用；返回的 `EditorRenderModel` 是当前帧的快照，SHOULD 视为不可变对象，MAY 在渲染线程安全读取 |
| `NewLineActionProvider` | **MUST** | `provideNewLineAction()` MUST 在 UI 线程同步调用和返回 |
...
### 12.1 `EditorCore` 生命周期

| 阶段 | 约束级别 | 规则 |
|---|---|---|
| 创建 | **MUST** | `EditorCore` 实例 MUST 在控件初始化阶段创建（命令式框架：构造函数或 init；声明式框架：控件首次挂载时） |
| 销毁 | **MUST** | 控件从视图树永久移除时 MUST 销毁 `EditorCore`，释放 C++ 侧内存 |
| 销毁后调用 | **MUST** | `EditorCore` 销毁后，任何方法调用 MUST 为 no-op 或抛出明确的"已销毁"异常，MUST NOT 访问已释放的 C++ 内存 |
| 重复销毁 | **MUST** | 多次调用销毁方法 MUST 为幂等操作（no-op），MUST NOT 导致 double-free |
...
| 控件销毁时清理 | **MUST** | 控件销毁时 MUST 自动注销所有已注册的 Provider，并取消所有进行中的异步请求（通过 `receiver.isCancelled()` 返回 true） |
...
### 12.3 `SweetEditorController` 生命周期（声明式框架）
...
控件销毁时，各子系统 MUST 按以下顺序释放：

```
1. 取消所有进行中的异步 Provider 请求
2. 注销所有 Provider（Decoration / Completion / NewLine）
3. 清除 EventBus 所有监听器
4. 销毁 EditorCore（释放 C++ 内存）
5. 释放平台特定资源（纹理、画布、定时器等）
```

> 步骤 1-3 的内部顺序 MAY 调整，但 MUST 在步骤 4 之前完成。步骤 4 MUST 在步骤 5 之前完成。
...
`Document` 是编辑器的核心数据类型，封装了 C++ 侧的文档句柄。

### 14.1 构造方式

各平台 MUST 至少支持以下两种构造方式：

| 构造方式 | 约束级别 | 说明 |
|---|---|---|
| 从字符串创建 | **MUST** | `Document(text: String)` — 从内存中的文本内容创建 |
| 从文件路径创建 | **SHOULD** | `Document(file: File)` / `Document(path: String)` — 从本地文件创建（使用内存映射优化大文件） |

> 构造函数的参数命名和类型 MAY 因平台而异（如 Java `File`、C# `string path`、Swift `URL`），但语义 MUST 一致。
...
| 规则 | 约束级别 | 说明 |
|---|---|---|
| 原生句柄 | **MUST** | `Document` 内部 MUST 持有 C++ 侧的文档句柄（`intptr_t`），所有操作通过桥接层委托给 C++ Core |
| 资源释放 | **MUST** | `Document` 销毁时 MUST 释放 C++ 侧的文档内存（Java: `close()` / `finalize()`、C#: `Dispose()`、Swift: `deinit`、Dart: `Finalizer`） |
| 编码模型 | **MUST** | C++ Core 内部使用 UTF-8 存储 + UTF-16 布局/测量的混合模型；平台层 MUST NOT 假设特定的内部编码，MUST 通过公共 API 获取文本 |
| 行尾符 | **MUST** | C++ Core 支持 LF、CR、CRLF 三种行尾符；`getLineText()` 返回的文本 MUST NOT 包含行尾符 |

### 14.4 与 `loadDocument()` 的关系
...
## 15. Copilot / InlineSuggestion 接口定义（SHOULD）

内联建议（Copilot）模块为 SHOULD 级别，但实现时 MUST 遵循以下接口规范。

### 15.1 `InlineSuggestion` 数据类型
...
### 15.2 `InlineSuggestionListener` 接口
...
| `onSuggestionDismissed` | **MUST** | 用户拒绝建议时（Esc 键或 Dismiss 按钮） |
...
| `setInlineSuggestionListener(listener)` | **MUST** | 设置内联建议的 Accept/Dismiss 回调监听器 |
...
## 16. `EditorMetadata` 与 `LanguageConfiguration` 字段定义（MUST）

### 16.1 `EditorMetadata`

`EditorMetadata` 是一个**标记接口**（Marker Interface），不定义任何字段。

| 规则 | 约束级别 | 说明 |
|---|---|---|
| 接口定义 | **MUST** | 各平台 MUST 定义 `EditorMetadata` 为空接口（Java: `interface`、C#: `interface`、Swift: `protocol`、Dart: `abstract class`、ArkTS: `interface`） |
| 用途 | **MUST** | 宿主代码通过实现该接口定义自定义元数据（如文件路径、语言 ID 等），并通过 `setMetadata()` / `getMetadata()` 存取 |
| 类型转换 | **MUST** | `getMetadata()` 返回 `EditorMetadata?`，宿主代码 MUST 自行向下转型到具体子类型 |
...
| `brackets` | List\<BracketPair\> | **MUST** | 括号对列表（设置时自动同步到 Core 的 `setBracketPairs()`） |
...
| 括号同步 | **MUST** | 调用 `setLanguageConfiguration()` 时，平台层 MUST 自动将 `brackets` 同步到 Core 的 `setBracketPairs()` |

---

## 17. `CompletionItem` 字段定义（MUST）
...
基于 `perf/` 模块（`PerfOverlay`、`PerfStepRecorder`、`MeasurePerfStats`）和 C++ Core 的 `PERF_TIMER` 宏，定义跨平台的性能基准。

### 18.1 帧率与渲染耗时

| 指标 | 约束级别 | 目标值 | 说明 |
|---|---|---|---|
| 滚动帧率 | **SHOULD** | ≥ 60fps | 在 1 万行以内的文档中，滚动操作 SHOULD 保持 60fps |
| `buildRenderModel()` 耗时 | **SHOULD** | ≤ 8ms | 超过 8ms 标记为 SLOW（与各平台 `WARN_BUILD_MS` 一致） |
| 单帧绘制耗时 | **SHOULD** | ≤ 8ms | 超过 8ms 标记为 SLOW（与各平台 `WARN_PAINT_MS` 一致） |
| 单帧总耗时 | **SHOULD** | ≤ 16.6ms | 超过 16.6ms 标记为 SLOW FRAME |
| 单个渲染步骤 | **SHOULD** | ≤ 2ms | 超过 2ms 在 PerfOverlay 中标记 `!`（与 `WARN_PAINT_STEP_MS` 一致） |
| 输入路径耗时 | **SHOULD** | ≤ 3ms | 从手势/键盘事件到 Core 处理完成的耗时（与 `WARN_INPUT_MS` 一致） |

### 18.2 大文档性能指导

| 场景 | 约束级别 | 指导 |
|---|---|---|
| 10 万行文档加载 | **SHOULD** | 使用 mmap 或流式加载，避免一次性分配全部内存；加载时间 SHOULD ≤ 500ms |
| 10 万行文档滚动 | **SHOULD** | 依赖视口渲染（仅布局和绘制可见行），滚动帧率 SHOULD ≥ 30fps |
| 内存占用 | **SHOULD** | 10 万行文档的平台层内存占用 SHOULD ≤ 50MB（不含 C++ Core 文档存储） |

### 18.3 Provider 超时策略

| 规则 | 约束级别 | 说明 |
|---|---|---|
| `DecorationProvider` 超时 | **SHOULD** | 如果 `receiver.accept()` 在 5 秒内未被调用，Manager SHOULD 取消该请求（`receiver.isCancelled()` 返回 true） |
| `CompletionProvider` 超时 | **SHOULD** | 如果 `receiver.accept()` 在 3 秒内未被调用，Manager SHOULD 取消该请求 |
| `NewLineActionProvider` 超时 | **MUST** | `provideNewLineAction()` 为同步调用，MUST 在 1ms 内返回 |

### 18.4 `PerfOverlay` 调试面板（SHOULD）

| 规则 | 约束级别 | 说明 |
|---|---|---|
| API | **SHOULD** | 各平台 SHOULD 提供 `setPerfOverlayEnabled(bool)` / `isPerfOverlayEnabled()` API |
| 默认状态 | **MUST** | PerfOverlay MUST 默认关闭，仅用于调试 |
| 显示位置 | **SHOULD** | 启用时 SHOULD 在编辑区左上角显示半透明性能面板 |
| 显示内容 | **SHOULD** | SHOULD 至少包含：FPS、单帧总耗时/build耗时/draw耗时、文本测量统计 |
| 稳定性 | **MUST** | PerfOverlay 的字段名、阈值和步骤名为调试展示，MUST NOT 作为稳定 API 契约 |
