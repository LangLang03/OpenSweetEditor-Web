...
| `contentStartPadding` | float | 0 | `setContentStartPadding(padding)` | `getContentStartPadding()` | `relayout` | gutter 分割线与文本渲染起始之间的额外水平内边距（像素） |
...
> - `runtime-transition`：MUST 立即生效，并安全处理该设置要求的运行态切换。
>
...
| 事件 | 字段 | 说明 |
|---|---|---|
| `TextChangedEvent` | `action: TextChangeAction`, `changeRange: TextRange?`, `text: String?` | 文本变更动作、变更前的范围，以及插入/替换后的文本 |
...
| `FoldToggleEvent` | `line: int`, `isGutter: boolean`, `screenPoint: PointF` | 被切换的折叠行、是否来自 gutter 点击，以及屏幕坐标 |
| `GutterIconClickEvent` | `line: int`, `iconId: int`, `screenPoint: PointF` | 被点击的 gutter icon 所在行、icon id，以及屏幕坐标 |
| `InlayHintClickEvent` | `line: int`, `column: int`, `type: InlayType`, `intValue: int`, `screenPoint: PointF` | 被点击的 inlay 位置、inlay 类型、类型相关值、可选文本 payload，以及屏幕坐标 |
| `LongPressEvent` | `cursorPosition: TextPosition`, `screenPoint: PointF` | 长按命中的位置与屏幕坐标 |
| `DoubleTapEvent` | `cursorPosition: TextPosition`, `hasSelection: boolean`, `selection: TextRange?`, `screenPoint: PointF` | 双击命中的位置、结果选区状态，以及屏幕坐标 |
| `ContextMenuEvent` | `cursorPosition: TextPosition`, `screenPoint: PointF` | 上下文菜单命中的位置与屏幕坐标 |
