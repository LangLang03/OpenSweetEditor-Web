...
| **Foundation** | `TextPosition`, `TextRange`, `TextChange`, `WrapMode`, `FoldArrowMode`, `AutoIndentMode`, `CurrentLineRenderMode`, `ScrollBehavior` | 基础值类型与枚举 |
...
| **Event** | 类型安全事件机制、`EditorEvent`、`TextChangedEvent`、`CursorChangedEvent`、`SelectionChangedEvent`、`ScrollChangedEvent`、`ScaleChangedEvent`、`DocumentLoadedEvent`、`FoldToggleEvent`、`GutterIconClickEvent`、`InlayHintClickEvent`、`CodeLensClickEvent`、`LinkClickEvent`、`LongPressEvent`*(仅移动端)*、`DoubleTapEvent`、`ContextMenuEvent`*(支持 ContextMenu 的平台)*；若采用显式事件总线 / 监听器模式，推荐 `EditorEventBus`、`EditorEventListener` | 事件系统 |
...
| **ContextMenu** *(MAY)* | `ContextMenuController`、`ContextMenuRequest`、`ContextMenuSection`、`ContextMenuItem`、`ContextMenuItemProvider`、宿主可见的 custom item 点击回调机制；MAY: `ContextMenuPopup`、`ContextMenuTriggerKind` | 平台侧的上下文菜单 / 动作菜单（桌面端 SHOULD，移动端 MAY） |
...
| `ContextMenuItemProvider` | C#/TS/Kotlin: `IContextMenuItemProvider`; OC: `SEContextMenuItemProvider` | 上下文菜单项提供者；用于构建当前这一轮显示的完整菜单 |
...
| 显式 teardown（若提供） | **MAY** | 平台 MAY 提供 `dispose()` / `close()` / `release()` 等显式 controller teardown 方法。对于 GC 管理语言，若 `unbind()` 后 controller 可自然失活并被回收，则不强制额外暴露显式 teardown API |
| 显式 teardown 语义 | **MUST** | 若平台提供显式 controller teardown 方法，该方法 MUST 只释放 controller 自身持有的资源、解绑控件、停止定时器 / listener / receiver 并切断引用链；MUST NOT 假定 controller 拥有已绑定控件，也 MUST NOT 直接销毁 `View` / `Control` / `Widget` 本身 |
...
DoubleTapEvent,
ContextMenuEvent      // 支持 ContextMenu 的平台
```

> `LongPressEvent` 用于移动端（iOS/Android）。`ContextMenuEvent` 用于暴露平台侧上下文菜单入口的实现（例如桌面右键、跨平台框架的上下文手势、或移动端长按）。平台实现 SHOULD 仅注册与自身平台相关的事件。
...
| `FoldToggleEvent` | `line: int`, `isGutter: boolean`, `locationInView: PointF 或平台原生点类型` | 被切换的折叠行、是否来自 gutter 点击，以及相对 editor view 的点位 |
| `GutterIconClickEvent` | `line: int`, `iconId: int`, `locationInView: PointF 或平台原生点类型` | 被点击的 gutter icon 所在行、icon id，以及相对 editor view 的点位 |
| `InlayHintClickEvent` | `line: int`, `column: int`, `type: InlayType`, `intValue: int`, `locationInView: PointF 或平台原生点类型` | 被点击的 inlay 位置、inlay 类型、类型相关值，以及相对 editor view 的点位 |
| `CodeLensClickEvent` | `line: int`, `column: int`, `commandId: int`, `locationInView: PointF 或平台原生点类型` | 被点击的 CodeLens 行/列锚点、唯一命令 id，以及相对 editor view 的点位 |
| `LinkClickEvent` | `line: int`, `column: int`, `target: String`, `locationInView: PointF 或平台原生点类型` | 被点击的链接行/列锚点、解析后的链接目标，以及相对 editor view 的点位 |
| `LongPressEvent` | `cursorPosition: TextPosition`, `locationInView: PointF 或平台原生点类型` | 长按命中的位置与相对 editor view 的点位 |
| `DoubleTapEvent` | `cursorPosition: TextPosition`, `hasSelection: boolean`, `selection: TextRange?`, `locationInView: PointF 或平台原生点类型` | 双击命中的位置、结果选区状态，以及相对 editor view 的点位 |
| `ContextMenuEvent` | `cursorPosition: TextPosition`, `locationInView: PointF 或平台原生点类型` | 上下文菜单命中的位置与相对 editor view 的点位 |
...
| `pointerCursorType` | `PointerCursorType` | **MUST** | 当前鼠标位置对应的指针样式提示 |

> 桌面平台 SHOULD 在手势处理后立即应用 `pointerCursorType`，以获得及时的鼠标形状反馈。纯触摸平台 MAY 忽略视觉上的鼠标形状变化，但仍应保持枚举和桥接层兼容。

### 11.5 ContextMenu 标准契约

`ContextMenu` 是 widget 层、平台侧的 UI 能力。它 MUST NOT 被建模为 C++ Core 的 render model 概念，也 MUST NOT 作为 Core decoration 类型序列化。若平台实现了上下文菜单，SHOULD 暴露以下标准数据模型与语义。
...
| `locationInView` | `PointF 或平台原生点类型` | **MUST** | 相对 editor view 的指针位置 |
...
上下文菜单能力 SHOULD 满足以下语义：
- `ContextMenuEvent` 是触发信号；`ContextMenuRequest` 是用于构建真实菜单模型的不可变快照
- provider 返回的是当前这次显示应使用的完整菜单模型，而不是增量追加项
- 将 provider 设为 `null` 时 SHOULD 恢复平台默认上下文菜单
- provider 返回空列表时 MAY 不显示菜单
- `locationInView` MUST 始终保持 view-local 语义；平台只在真正展示 Popup 或原生菜单时再换算成 screen / window 坐标
...
- 若支持 custom item 注入，平台 MUST 提供宿主可见的方式，通过 `ContextMenuItemClickEvent` 或等价回调 payload 观察 custom item 被触发
- 在命令执行、文本变化、或其他会使当前目标失效的手势之后，菜单 SHOULD 关闭；除非平台有意支持持久化的多步工作流

平台 SHOULD 暴露语义等价于 `setContextMenuItemProvider(provider)` 的 `SweetEditor` 级 API，以便宿主自定义上下文菜单内容。
...
| 资源释放路径 | **MUST** | 平台 MUST 保证 `EditorCore` 及其 native / C++ 侧资源最终会被释放。对于 GC 管理语言，标准的主要合规目标是“逻辑 teardown + native 最终回收”，而不是强制要求显式 release 方法。释放机制 MAY 是显式 `dispose()` / `close()`、宿主管理生命周期、基于 GC / finalizer / ARC 的自动回收、平台等价清理钩子，或其他平台惯用策略。view detach、widget unmount 或视图树中的临时移除，本身 NOT 必须构成最终回收时机 |
| teardown 后调用 | **MUST** | 若平台暴露显式释放 API，或对象在逻辑 teardown / 内部释放后仍可能继续被访问，则后续调用 MUST NOT 访问已释放的 native / C++ 资源，且 MUST NOT 再触发编辑器副作用或回调。变更类调用 MUST 为 no-op 或抛出明确的"已销毁"异常。getter 调用 MAY 返回 `null`、默认值，或托管侧最后一次已知快照，只要这些返回不依赖已释放的 native 状态，且不会触发针对已销毁资源的惰性重计算 |
| 重复释放 | **MUST** | 若平台暴露显式释放逻辑，多次调用 MUST 为幂等操作（no-op），MUST NOT 导致 double-free |

> 标准要求的是 native 资源最终释放，但**不要求**托管语言中的每个 `Document` / bridge wrapper 都额外暴露一个显式 release API；若平台已有自身生命周期模型，应优先遵循该模型。对于 GC 语言，平台 SHOULD 优先保证逻辑 teardown：停止定时器、解绑 listener、取消或标记异步 receiver 为 stale，并切断原本会阻止 editor 对象图被回收的引用链。若平台在 teardown 后仍返回托管侧最后一次已知快照，SHOULD 在文档中明确这些值是陈旧快照，而不是仍可操作的实时 editor 状态。
...
| 释放阶段清理 | **MUST** | 若平台定义了显式的 editor 释放 / dispose / close / teardown 阶段，则 MUST 自动注销所有已注册的 Provider，并取消或标记所有进行中的异步请求为过期，使延迟结果被忽略。对于 GC 管理平台，即使完全依赖自动回收，也 MUST 在等价的逻辑 teardown 中解绑 listener、停止定时器，并取消或标记异步 receiver 为 stale，确保延迟结果既不会阻止 editor 对象图被回收，也不会修改已释放的 native 资源或宿主可见的 editor 状态 |
...
| 显式 teardown（若提供） | **MAY** | 平台 MAY 提供 `dispose()` / `close()` / `release()` 等显式 controller teardown 方法；对于 GC 管理语言，若 controller 在 `unbind()` 后不会继续持有活跃资源或引用链，则不强制额外提供 |
| teardown 顺序与边界 | **MUST** | 若平台提供显式 controller teardown 方法，该方法 MUST 先解绑控件（如果仍绑定），再释放 controller 自身持有的内部资源；teardown 后任何方法调用 MUST 为 no-op。controller MUST NOT 假定自己拥有绑定的控件，也 MUST NOT 直接销毁 `View` / `Control` / `Widget` 本身 |
| 控件重建 | **SHOULD** | 声明式框架中控件可能因 state 变化被重建；Controller SHOULD 在旧控件 `unbind()` 后无缝 `bind()` 到新控件，不丢失排队的操作 |

> 本节只适用于存在独立 controller 对象的平台；不得据此要求所有命令式 `View` / `Control` / `Widget` / `Document` 类型都新增库自定义的 `dispose()` / `close()` 方法。controller 的 teardown 语义是“解绑与逻辑失活”，不是接管或销毁其绑定的控件生命周期。
...
- Provider 注册 MUST 在其继续向已销毁 editor 发出回调之前被清除
...
| 字段 | 类型 | MUST/MAY | 说明 |
|---|---|---|---|
| `pointerCursorType` | PointerCursorType | **MUST** | 当前快照中鼠标位置对应的指针样式提示 |

> 桌面平台 SHOULD 将 `pointerCursorType` 映射到原生鼠标形状，通常文本编辑区域使用 `TEXT`，可点击交互内容使用 `HAND`，滚动条或 gutter 等中性区域在合适时使用 `DEFAULT`。

> `GestureResult.pointerCursorType` 与 `EditorRenderModel.pointerCursorType` SHOULD 保持语义一致。平台 MAY 用前者做即时鼠标更新，用后者作为最新稳定快照状态。
...
| 资源释放 | **MUST** | 当 `Document` 到达其平台生命周期中的终止状态时，桥接层 MUST 最终释放 C++ 侧文档内存；具体清理机制由平台决定，显式 `dispose()` / `close()` API 为可选 |
